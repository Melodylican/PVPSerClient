...
public class ZmqCliProxy {
	private static final Logger logger = Logger.getLogger(ZmqCliProxy.class);

	private ZContext ctx; // Our context wrapper
	private Socket spipe; // spipe through to clone agent

	// .split constructor and destructor
	// Here are the constructor and destructor for the clone class. Note that
	// we create a context specifically for the spipe that connects our
	// frontend to the backend agent:
	public ZmqCliProxy() {
		ctx = new ZContext();
		spipe = ZThread.fork(ctx, new CloneAgent());
	}

	public ZmqCliProxy(ZmqHandlerMsg hmsg) {
		ctx = new ZContext();
		spipe = ZThread.fork(ctx, new CloneAgent(hmsg));
	}
	...
	public void send(String Subtree, String value, int ttl) {
		set(Subtree,value,ttl);
	}
	
	public void send(String Subtree, byte[] value, int ttl) {
		set(Subtree,value,ttl);
	}
	...
	private static class Server {
		...
		protected Server(ZContext ctx, String address, int port, String subtree) {
			logger.info("I: adding server " + address + " " + port);
	...
	private static class Agent {
		...
		private Socket spipe; // spipe back to application
		...
		protected Agent(ZContext ctx, Socket spipe) {
			this.ctx = ctx;
			this.spipe = spipe;
			kvmap = new HashMap<String, String>();
			subtree = "";
			state = STATE_INITIAL;
			publisher = ctx.createSocket(ZMQ.PUSH);
		...
		private boolean controlMessage() {
			...
			if (command.equals("SUBTREE")) {
				...
				if (nbrServers < SERVER_MAX) {
					...
					logger.info(String.format("%s:%d", address, Integer.parseInt(service) + 2));
				} else
					logger.error("too many servers " + SERVER_MAX);
			} else
	...
	private static class CloneAgent implements IAttachedRunnable {
		private ZmqHandlerMsg hmsg;

		protected CloneAgent(ZmqHandlerMsg hmsg) {
			this.hmsg = hmsg;
		}

		protected CloneAgent() {
		}

		public void run(Object[] args, ZContext ctx, Socket spipe) {
			Agent self = new Agent(ctx, spipe);

			while (!Thread.currentThread().isInterrupted()) {
				PollItem[] pollItems = { new PollItem(spipe, Poller.POLLIN), null };
				...
				case STATE_INITIAL:
					...
					if (self.nbrServers > 0) {
						logger.info("I: waiting for server at " + server.address + " " + server.port);
				...
				if (null != pollItems[1] && pollItems[1].isReadable()) {
					...
					if (self.state == STATE_SYNCING) {
						...
						if (msg.getKey().equals("KTHXBAI")) {
							...
							if (hmsg != null) {
								...
								logger.warn(String.format("W hmsg is null,received from %s:%d snapshot=%d",
										server.address, server.port, self.sequence));
							}
						}
						...
						if (msg.getSequence() > self.sequence) {
							if (hmsg != null) {
								...
								logger.warn(String.format("W: received from %s:%d update=%d", server.address,
										server.port, self.sequence));
							}
							self.sequence = msg.getSequence();
						} else {

							logger.info(String.format("I: received from %s:%d hasupdated=%d curupdate=%d",
									server.address, server.port, self.sequence, msg.getSequence()));
						}
